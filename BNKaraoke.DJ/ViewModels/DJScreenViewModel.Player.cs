...
namespace BNKaraoke.DJ.ViewModels
{
    public partial class DJScreenViewModel
    {
        ...
        private const string FadeButtonDefaultBackgroundHex = "#374151";
        private const string FadeButtonPlayingBackgroundHex = "#DC2626";
        private const string FadeButtonFadingBackgroundHex = "#FBBF24";
        private const string FadeButtonDefaultForegroundHex = "White";
        private const string FadeButtonActiveForegroundHex = "Black";
        ...
        [ObservableProperty] private double _masterVolume = SettingsService.Instance.Settings.MasterVolume;
        [ObservableProperty] private string _fadeOutButtonBackground = FadeButtonDefaultBackgroundHex;
        [ObservableProperty] private string _fadeOutButtonForeground = FadeButtonDefaultForegroundHex;
        [ObservableProperty] private bool _isFadeOutButtonEnabled;

        partial void OnIsPlayingChanged(bool value)
        {
            RefreshAudioRecoveryCommands();
            RefreshFadeButtonState();
        }
        ...
        private void RefreshFadeButtonState()
        {
            var hasActivePlayback = HasActivePlayback();
            IsFadeOutButtonEnabled = hasActivePlayback;

            if (_manualFadeActive)
            {
                FadeOutButtonBackground = FadeButtonFadingBackgroundHex;
                FadeOutButtonForeground = FadeButtonActiveForegroundHex;
                return;
            }

            if (IsPlaying)
            {
                FadeOutButtonBackground = FadeButtonPlayingBackgroundHex;
                FadeOutButtonForeground = FadeButtonActiveForegroundHex;
                return;
            }

            FadeOutButtonBackground = FadeButtonDefaultBackgroundHex;
            FadeOutButtonForeground = FadeButtonDefaultForegroundHex;
        }
        ...
        private bool HasActivePlayback()
        {
            ...
            if (_videoPlayerWindow.MediaPlayer.State == VLCState.Stopped)
            {
                return false;
            }

            return PlayingQueueEntry != null;
        }
        ...
        {
            var effectiveVolume = Math.Clamp(value, 0, 100);
            if (effectiveVolume != value)
            {
                _masterVolume = effectiveVolume;
                OnPropertyChanged(nameof(MasterVolume));
            }

            _settingsService.Settings.MasterVolume = effectiveVolume;
            try
            {
            ...
            {
                Log.Warning(ex, "[VOLUME] Failed to persist master volume: {Message}", ex.Message);
            }

            ApplyMasterVolumeChange();
        }

        private void ApplyMasterVolumeChange()
        {
            RefreshPlaybackVolumes(PlayingQueueEntry?.NormalizationGain);
            UpdateMediaPlayerVolumeFromPreFade();
        }
                ...
                Application.Current.Dispatcher.InvokeAsync(() =>
                {
                    if ((IsPlaying || IsVideoPaused) && _totalDuration.HasValue && _videoPlayerWindow?.MediaPlayer != null)
                    {
                        try
                        {
                            ...
                            var manualFadeRemaining = CalculateManualFadeRemaining(currentTime.TotalSeconds);

                            if (manualFadeRemaining.HasValue)
                            {
                                var fadeSeconds = (int)Math.Max(0, Math.Ceiling(manualFadeRemaining.Value));
                                TimeRemainingSeconds = fadeSeconds;
                                TimeRemaining = TimeSpan.FromSeconds(manualFadeRemaining.Value).ToString(@"m\:ss");
                            }
                            else
                            {
                            ...
                            if (manualFadeRemaining.HasValue)
                            {
                                if (manualFadeRemaining.Value <= 0)
                                {
                                    Log.Information("[DJSCREEN] Countdown ended (manual fade)");
        ...
        private double? CalculateManualFadeRemaining(double currentSeconds)
        {
            ...
            var elapsed = currentSeconds - _fadeStartTimeSeconds.Value;
            ...
            lock (_stateLock)
            {
                Application.Current.Dispatcher.InvokeAsync(() =>
                {
                    try
                    {
                        ...
                        ReapplyIntroMuteAfterSeek(_pendingSeekPosition);
        ...
        private void ApplyIntroMuteStateInternal()
        {
            ...
            var remaining = Math.Max(0, _introMuteSeconds.Value - currentSeconds);
            ...
            Log.Information("[AUDIO] Intro mute armed for {Seconds}s (QueueId={QueueId})", remaining, queueId);
            _videoPlayerWindow.MediaPlayer.Mute = true;
            _videoPlayerWindow.MediaPlayer.Volume = 0;

            _introMuteTimer = new DispatcherTimer
            {
                Interval = TimeSpan.FromSeconds(remaining)
            };
        ...
        private void ReapplyIntroMuteAfterSeek(double positionSeconds)
        {
            if (_videoPlayerWindow?.MediaPlayer == null)
            {
                return;
            ...
            var configuredIntroMute = PlayingQueueEntry?.IntroMuteDuration;
            if (!configuredIntroMute.HasValue || configuredIntroMute.Value <= 0)
            {
                CancelIntroMuteTimer();
                _introMuteSeconds = null;
                _videoPlayerWindow.MediaPlayer.Mute = false;
            ...
            var remaining = configuredIntroMute.Value - positionSeconds;
            if (remaining > 0.05)
            {
                _introMuteSeconds = configuredIntroMute.Value;
                ApplyIntroMuteState();
            }
            else
            {
        ...
        private void VideoPlayerWindow_MediaPlayerReinitialized(object? sender, EventArgs? e)
        {
            ...
            {
                RefreshPlaybackVolumes(PlayingQueueEntry?.NormalizationGain);
                Log.Information("[VOLUME] No previous volume detected during reinitialize. Defaulting to {Volume}%", _preFadeVolume);
            }
            _videoPlayerWindow.MediaPlayer.Volume = ClampVolume(_preFadeVolume);
            Log.Information("[VOLUME] MediaPlayer reinitialized -> Volume={Volume}%", _preFadeVolume);
            var currentSeconds = _videoPlayerWindow.MediaPlayer.Time / 1000.0;
            ReapplyIntroMuteAfterSeek(currentSeconds);
        }
        ...
        private void ClampAutoFadeStartToDuration()
        {
            if (_manualFadeActive || !_fullSongDuration.HasValue || !_fadeStartTimeSeconds.HasValue)
            {
                return;
            }

            var maxFadeStart = Math.Max(0, _fullSongDuration.Value.TotalSeconds - 7);
            if (_fadeStartTimeSeconds.Value <= maxFadeStart)
            {
                return;
            }

            _fadeStartTimeSeconds = maxFadeStart;
            if (_configuredFadeStartSeconds.HasValue)
            {
                _configuredFadeStartSeconds = Math.Min(_configuredFadeStartSeconds.Value, maxFadeStart);
            }

            Log.Information("[DJSCREEN] Clamped fade start time to {FadeStart:F2}s because duration is {Duration:F2}s",
                _fadeStartTimeSeconds, _fullSongDuration.Value.TotalSeconds);
        }

        private double CalculateBaseVolume(float? gain)
        {
            var multiplier = gain.HasValue ? Math.Pow(10, gain.Value / 20.0) : 1.0;
            var baseVolume = MasterVolume * multiplier;
            return Math.Clamp(baseVolume, 0, 200);
        }

        private void RefreshPlaybackVolumes(float? normalizationGain)
        {
            _baseVolume = CalculateBaseVolume(normalizationGain);
        ...
        private void UpdateMediaPlayerVolumeFromPreFade()
        {
            if (_videoPlayerWindow?.MediaPlayer == null)
            {
                return;
        ...
        private async Task PlayAsync()
        {
            ...
            try
            {
                ...
                if (_preFadeVolume < 0)
                {
                    RefreshPlaybackVolumes(targetEntry.NormalizationGain);
                    Log.Information("[VOLUME] No previous volume detected during playback. Defaulting to {Volume}%", _preFadeVolume);
                }

                _videoPlayerWindow.MediaPlayer.Volume = ClampVolume(_preFadeVolume);
                Log.Information("[VOLUME] VideoPlayer ready for playback -> Volume={Volume}%", _preFadeVolume);
                ...
                try
                {
                    ...
                    var startMuted = targetEntry.IntroMuteDuration.HasValue && targetEntry.IntroMuteDuration.Value > 0;
                    _videoPlayerWindow.PlayVideo(videoPath, startMuted: startMuted);
                    ...
                    if (_videoPlayerWindow?.MediaPlayer == null)
                    {
                        Log.Error("[DJSCREEN] MediaPlayer not ready after PlayVideo for QueueId={QueueId}", targetEntry.QueueId);
                        await SetWarningMessageAsync("Video player not ready. Please restart the show.");
                        await Application.Current.Dispatcher.InvokeAsync(() =>
                        {
                            IsPlaying = false;
                            IsVideoPaused = true;
        ...
        private async Task StopRestartAsync()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        RefreshPlaybackVolumes(PlayingQueueEntry.NormalizationGain);
                        Log.Information("[VOLUME] Stop/Restart recalculated pre-fade volume={Volume}%", _preFadeVolume);
                    ...
                    var startMuted = PlayingQueueEntry.IntroMuteDuration.HasValue && PlayingQueueEntry.IntroMuteDuration.Value > 0;
                    _videoPlayerWindow.RestartVideo(startMuted);
                    if (_updateTimer == null)
                    {
                        _updateTimer = new DispatcherTimer { Interval = TimeSpan.FromSeconds(1) };
                        _updateTimer.Tick += UpdateTimer_Tick;
        ...
        private async Task TriggerManualFadeOutAsync()
        {
            ...
            try
            {
                ...
                if (mediaPlayer == null)
                {
                    Log.Warning("[DJSCREEN] Manual fade skipped: MediaPlayer unexpectedly null");
                    return;
                }
