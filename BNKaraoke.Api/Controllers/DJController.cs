    ...
    public class DJController : ControllerBase
    {
        ...
        public async Task<IActionResult> PlaySong(int eventId, int queueId)
        {
            try
            {
                ...
                var queueItemV2 = await BroadcastQueueItemAsync(queueDto, "Playing", "QueueItemUpdatedV2", user, singerStatus);
                await BroadcastNowPlayingAsync(eventId, queueItemV2);
        ...
        public async Task<IActionResult> SkipSong(int eventId, int queueId)
        {
            try
            {
                ...
                await BroadcastQueueItemAsync(queueDto, "Skipped", "QueueItemUpdatedV2", user, singerStatus);
                if (wasPlaying)
                {
        ...
        public async Task<IActionResult> SetNowPlaying([FromBody] NowPlayingDto request)
        {
            try
            {
                ...
                await BroadcastSungCountAsync(queueEntry.EventId);
                var singersList = new List<string>();
                try
                {
                    singersList.AddRange(JsonSerializer.Deserialize<List<string>>(queueEntry.Singers) ?? new List<string>());
                ...
                var queueItemV2 = await BroadcastQueueItemAsync(queueDto, "Playing", "QueueItemUpdatedV2", user, singerStatus);
                await BroadcastNowPlayingAsync(queueEntry.EventId, queueItemV2);
        ...
        public async Task<IActionResult> ResetNowPlaying(int eventId)
        {
            try
            {
                ...
                {
                    await BroadcastQueueItemAsync(dto, "Reset", "QueueItemUpdatedV2", null, null);
                }
                await BroadcastNowPlayingAsync(eventId, null);
            ...
            try
            {
                ...
                foreach (var entry in queueEntries)
                {
                    ...
                    else
                    {
                        ...
                        await BroadcastQueueItemAsync(queueDto, "OnHold", "QueueItemUpdatedV2", user, singerStatus);
                    }
                }
                if (nextEntry == null)
                {
                ...
                var queueItemV2 = await BroadcastQueueItemAsync(nextQueueDto, "Playing", "QueueItemUpdatedV2", nextUser, nextSingerStatus);
                await BroadcastNowPlayingAsync(eventId, queueItemV2);
        ...
        public async Task<IActionResult> CompleteSong([FromBody] CompleteSongDto request)
        {
            try
            {
                ...
                eventEntity.SongsCompleted++;
                await _context.SaveChangesAsync();
                await BroadcastSungCountAsync(request.EventId);
                var singersList = new List<string>();
                try
                {
                    singersList.AddRange(JsonSerializer.Deserialize<List<string>>(queueEntry.Singers) ?? new List<string>());
                ...
                await BroadcastQueueItemAsync(queueDto, "Sung", "QueueItemUpdatedV2", user, singerStatus);
                if (wasPlaying)
                {
                    await BroadcastNowPlayingAsync(request.EventId, null);
                }
        ...
        public async Task<IActionResult> ToggleBreak([FromBody] ToggleBreakDto request)
        {
            try
            {
                ...
                await BroadcastQueueItemAsync(queueDto, request.IsOnBreak ? "OnHold" : "Eligible", "QueueItemUpdatedV2", user, singerStatus);
            ...
            try
            {
                ...
                using (var scope = new TransactionScope(TransactionScopeOption.Required, TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    foreach (var entry in queueEntries)
                    {
                        ...
                        var v1Action = !string.IsNullOrEmpty(holdReason) ? "Held" : "Eligible";
                        await BroadcastQueueItemAsync(queueDto, v1Action, "QueueItemUpdatedV2", user, dbSingerStatus);
                    }
                    await _context.SaveChangesAsync();
        ...
        private async Task<DJQueueItemDto> BroadcastQueueItemAsync(
            EventQueueDto queueDto,
            string legacyAction,
            string v2Message,
            ApplicationUser? requestor,
            SingerStatus? singerStatus)
        {
            var eventId = queueDto.EventId;
            await _hubContext.Clients.Group($"Event_{eventId}").SendAsync("QueueUpdated", new { data = queueDto, action = legacyAction });

            var user = requestor;
            if (user == null && !string.IsNullOrEmpty(queueDto.RequestorUserName))
            {
            ...
            var snapshot = status != null
                ? new SingerStatusData(
                    queueDto.RequestorUserName,
                    queueDto.RequestorFullName ?? queueDto.RequestorUserName,
                    status.IsLoggedIn,
                    status.IsJoined,
                    status.IsOnBreak)
                : null;

            var singerDto = DJQueueItemBuilder.BuildSingerStatus(queueDto, snapshot, userLookup);
            var queueItemV2 = DJQueueItemBuilder.BuildDjQueueItem(queueDto, singerDto);

            await _hubContext.Clients.Group($"Event_{eventId}").SendAsync(v2Message, queueItemV2);
            _logger.LogInformation("[DJController] Sent {V2Message} for EventId={EventId}, QueueId={QueueId}", v2Message, eventId, queueDto.QueueId);
            return queueItemV2;
        }
        ...
        private async Task BroadcastNowPlayingAsync(int eventId, DJQueueItemDto? nowPlaying)
        {
            await _hubContext.Clients.Group($"Event_{eventId}")
                .SendAsync("NowPlayingChanged", new { eventId, queueItem = nowPlaying });
        }
